...
void BinaryTree::listarEmOrdem()
{
    std::cout << "\n--- Impressão da árvore em ordem ---\n";
    listarEmOrdemRecursive(root);
}

void BinaryTree::listarEmOrdemRecursive(BNode *node)
{
    if (!node)
        return;

    listarEmOrdemRecursive(node->esq); // Visita esquerda
    if (!node->data.empty())           // Evita imprimir nós vazios
        std::cout << node->data << "\n\n";
    listarEmOrdemRecursive(node->dir); // Visita direita
}